                    ...
                    for w in widgets:
                        ...
                        if w.field_name and w.field_name.startswith("Image") and w.field_name.endswith("_af_image"):
                            try:
                                ...
                                pix = page.get_pixmap(clip=w.rect, dpi=500)  # 🔥 AUMENTATO DA 150 A 500
                                if pix:
...
def analizza_con_gemini_vision(img_rgb, lente="50x", luce="Bianca", zona="Vertice", parametri_cliente=None, api_key=None):
    ...
    try:
        ...
        prompt = f"""
Sei un medico tricologo esperto di tricoscopia avanzata per lo Studio Tricologico Righetti Since 1967.
Analizza con rigore clinico questa immagine tricoscopica ({lente}, luce {luce}, zona {zona}).
Parametri anamnestici: {parametri_cliente if parametri_cliente else 'Standard'}.

Restituisci ESCLUSIVAMENTE un JSON valido (senza markdown o altro testo) con questa struttura esatta:
{{
  "calibro_medio": <float, es. 72.5>,
  "anisotropia": <float, es. 22.0>,
  "densita": <int, stima per cm2, es. 145 a 50x, o valore coerente a 200x>,
  "tappi_sebacei": <int, numero reale tappi o manicotti cheratinici nel fotogramma>,
  "follicoli_silenti": <int, osti vuoti reali, a 200x max 2>,
  "eritemi": <int, grado o focolai di iperemia/rossore cutaneo, es. 1 o 2>,
  "descrizione_sintetica": "<testo clinico nei 4 punti standard>"
}}

REGOLE PER LA DESCRIZIONE SINTETICA:
1. Inquadramento & Cute: descrivi lo stato reale del microambiente cutaneo, grado di iperemia/eritema e idrolipidico.
2. Osti & Ancoraggio: descrivi pervietà ostiale, presenza di ipercheratosi/manicotti o tappi sebacei ed eventuali osti vuoti.
3. Fusti & Densità: cita calibro, anisotropia e densità stimata.
...
# ============================================================================
# RITAGLIO BORDO BIANCO DELLE IMMAGINI
# ============================================================================
def ritaglia_immagine(img_rgb):
    """Rimuove il bordo bianco dall'immagine"""
    try:
        ...
        print(f"⚠️ Errore ritaglio immagine: {e}")
        return img_rgb
                ...
                if modalita_confronto == "📊 Confronto Automatico da Storico Database (Supabase)":
                    ...
                    if uploaded_pdf is not None:
                        ...
                        if immagini_estratti:
                            ...
                            # Mostra le immagini in una griglia (CON RITAGLIO)
                            cols = st.columns(3)
                            for idx, img_data in enumerate(immagini_estratti):
                                # 🔥 RITAGLIA IL BORDO BIANCO PRIMA DI MOSTRARE
                                img_ritagliata = ritaglia_immagine(img_data["immagine"])
                                with cols[idx % 3]:
                                    st.image(
                                        img_ritagliata,
                            ...
                            for idx, img_data in enumerate(immagini_estratti):
                                # 🔥 PRENDI L'IMMAGINE ORIGINALE E RITAGLIALA
                                img_rgb = img_data["immagine"]
                                img_ritagliata = ritaglia_immagine(img_rgb)
                                
                                # Riduci la dimensione per velocizzare (max 1200px)
                                ...
                                # Converti in JPEG in memoria (senza salvarlo su disco)
                                ...
                                # Analizza l'immagine con il motore tricoscopico
